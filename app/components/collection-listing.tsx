...
export const CollectionListing = ({ post }) => {
  ...
  return (
    <Wrapper>
      <x.article color={color}>
        {post.books?.map((book) => (
          <BookListing key={book._id} book={book} spineColor={color} />
        ))}
        <x.header
          display={'flex'}
          flexDirection={post.books?.length < 8 ? 'column' : 'row'}
          borderTop={'1px solid'}
          mt={2}
          pt={2}
        >
          <x.div flex={'auto'}>
            <x.h2 color={color} mb={0} maxW={960}>
              {post.title}
            </x.h2>
            {post.subtitle ? <x.h4 mb={0}>{post.subtitle}</x.h4> : null}
          </x.div>
          <x.h4 color={color} textAlign={'left'} mb={0} flexShrink={0}>
            {post.display_date}
          </x.h4>
        </x.header>
      </x.article>

...
export const BookListing = ({ book, spineColor }) => {
  const { addModals } = useModal()
  const [loaded, setLoaded] = useState<boolean>(false)
  const [color, setColor] = useState<string>('')
  const [hFactor, setHFactor] = useState<number>(1)
  const [wFactor, setWFactor] = useState<number>(1)

  useEffect(() => {
    setColor(colors[(colors.length * Math.random()) | 0])
    setHFactor(Math.random() * (1.1 - 0.9) + 0.9)
    setWFactor(Math.random() * (1.35 - 0.65) + 0.65)
    setLoaded(true)
  }, [])
